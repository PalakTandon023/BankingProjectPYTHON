...
##The Open_acc() function is designed to allow users to open a new bank account and store the account details
# in a binary file named "master_file.dat".
def Open_acc():
    ch = 'y'
...
##The view_rec() function is designed to read and display the details of all
# bank accounts stored in the binary file "master_file.dat".
def view_rec():
...
#The withdrawl() function is designed to allow users to make a withdrawal from their bank account.
def withdrawl():
    pos = found = 0
    with open("C:\\sskp\\banking\\master_file.dat", "rb+") as fobj:
...
#The deposit() function is designed to allow users to make a deposit into their bank account.
def get_name(tacno):
    with open("C:\\sskp\\banking\\master_file.dat", "rb") as fobj:
        try:
            while fobj:
...
#The pri_pass() function is designed to print the passbook entry for a specific bank account.
def pri_pass():
...
#The pri_ledg() function is designed to print the ledger status, summarizing the deposit information for all accounts.
def pri_ledg():
    print("                                          STATE BANK OF INDIA          ")
    star()
    print("                                             LEDGER STATUS          ")
